...
const PRODUCT_SEARCH_API = 'https://www.onliner.by/sdapi/catalog.api/search/products'
...
describe('API Search Products', () => {
    [
        ...
        it(`GET validate response status for '${searchItem}'`, () => {
            cy.request(PRODUCT_SEARCH_API, {query: searchItem})
                .then((response) => {
        ...
        it(`GET validate response schema for '${searchItem}'`, () => {
            cy.request(PRODUCT_SEARCH_API, {query: searchItem})
                .then((response) => {
                    const result = validator.validate(response.data, productSearchSchema)
